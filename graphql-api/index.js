...
const { MONGODB_URI, MONGODB_URI2 } = require('./config');
...
async function connect() {
  ...
  const heroes = connection.db('heroes').collection('characters');
  return heroes;
}

// 2o in GraphQL world, we have a concept called by resolvers
// the resolvers are responsible to map our request from request -> server
...
const resolvers = {
  ...
  Query: {
    ...
    async getHeroes(parent, args, context) {
      const heroesCollection = context.db.heroes.find(args).toArray();
      //  const { name } = args;
      //  we can get all fields to use as filter
      // is just pass to find function and convert the _id to ObjectID
  ...
  Mutation: {
    async createHero(parent, args, context) {
      const {
        ops: [
          {
            // ES6 Syntax for extrating first item from array
            _id
          }
        ]
...
const schema = gql`
  ...
  type Query {
    getHeroes(name: String, _id: ID, power: String, birthday: String): [Heroes]
  }

  # Mutations are used to perform POST data
  # CREATE, UPDATE, DELETE
  type Mutation {
    createHero(name: String!, power: [String]!, birthday: String!): ID!
  }
`;
